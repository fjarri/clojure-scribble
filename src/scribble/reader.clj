...
(defn here-markers
  ...
  (if (nil? here-str)
    ...
    [
      ...
      (str scribble-text-end (clojure.string/join (inverse-vec (vec here-str))))]))
  ...
  (loop [text-accum []
    ...
    (let [c (reader-methods/read-1 reader)]
      (cond
        ...
        (and escaped (= c scribble-verbatim-end) (= (- (:here-str-pos state)) here-marker-len))
        ...
        (and escaped (= c scribble-verbatim-start))
          (let [[text-accum str-accum] (if (:leading-ws state)
        ...
        (and escaped (= c scribble-text-start) (= (:here-str-pos state) here-marker-len))
          (recur text-accum (conj str-accum c)
            (assoc state :here-str-pos 0 :brace-level (inc (:brace-level state))))

        (and escaped (= c scribble-char) (= (:here-str-pos state) here-marker-len))
        ...
        (= c \newline)
          (let [text-accum
                 (-> text-accum
                   ...
                   append-newline)]
            (recur text-accum [] (assoc state :leading-ws true :here-str-pos 0)))

        ; in leading whitespace mode, whitespace character encountered
        (and (whitespace? c) (:leading-ws state))
          (recur text-accum (conj str-accum c) (assoc state :leading-ws true :here-str-pos 0))
        (:leading-ws state)
          (recur (dump-leading-ws text-accum str-accum) [c]
    ...
    (let [c (reader-methods/read-1 reader)]
      (cond
        ...
        (= c scribble-text-start)
          (recur (conj forms-read (scribble-text-block-reader reader nil)) true)
